...
//}
    ...
    for (var i = 0; i < soils.length-1; i++){soils[i].cross();}
    consumer.display();
...
function Soil(centX, centY, stro,dist){
  ...
  this.cross = function(){
    ...
    sketch.line(this.centX+this.d,this.centY-this.d,this.centX-this.d,this.centY+this.d);
    if (this.trans>0){this.trans-=0.01;}
    else{
      if (this.thing_trigger == false){ // means last iff just ended, time to initilize the things
        this.thing_trigger =  true;
        for (var i=0;i<=5;i++){
          this.things[i] = new Thinggy(this.centX,this.centY);
        }
      }  /// end if thing initilization
      else{
        if (this.b_count<25){    // populate the bacterias
          for (var i=0;i<=5;i++){
            this.things[i].grow();
            this.bacteria[this.b_count] = new p5.Vector( this.things[i].poit.x, this.things[i].poit.y);
            this.b_count +=1;
        }
      }
      for (var i=0;i<this.b_count;i++){
        sketch.strokeWeight(.5);
        sketch.stroke(sketch.random(90,120),59,sketch.random(8,12),1);
        sketch.fill(sketch.random(90,120),59,sketch.random(25,35),1);
        sketch.ellipse(this.bacteria[i].x, this.bacteria[i].y,8,8);
      }
    }
    sketch.noStroke();
    }
...
function Thinggy(x,y) {
    this.poit = new p5.Vector(x,y);
    this.move = new p5.Vector(-1,1);
    //////////////base function
    this.grow = function() {
      this.move = new p5.Vector(sketch.random(-6,6),sketch.random(-6,6));
      this.poit.add(this.move);  // add random vecotr "move" to the actual
                      // while below limmit add to master
      }
      //////////////end function
}
...
function Settler(r,x,y){
    ...
    this.display = function() {
      ...
      for (rad=10; rad<this.r_sz/2; rad+=35){
        ...
        if (rad<this.r_sz*.2 && this.r_sz<100){soils[soil_loc] = new Soil(xxx+sketch.mouseX,yyy+sketch.mouseY, 1,10);  soil_loc +=1; if (soil_loc == soil_count){soil_loc = 0;}}
      }
      //sketch.stroke(0);
    }//end display functoin


    this.motionTest = function(){ /// this is where size limmit is
        change = sketch.dist(this.lX,this.lY,sketch.mouseX,sketch.mouseY);
        this.lX = sketch.mouseX;
        this.lY = sketch.mouseY;
        if (change < 4 ){if(this.r_sz < 400){this.r_sz += 3;}} ///this is size limmit
